...
def main(args):
    ...
    if args.time != None:
        mode = "stopByTime"
        duration = args.time
        total_sample = duration * fs
        total_frames = math.ceil(total_sample / chunk)
        start_idx = skip_frame
    ...
    while loop:
        ...
        elif mode == "stopByTime":
            if frame_count - skip_frame == total_frames * 2:
                break
            else:
                frame_count += 1
    ...
    with wave.open(filename, "wb") as wf:
        ...
        if mode == "endpointDetect":
            ...
            wf.writeframes(b''.join(frames[start_idx:end_idx]))
